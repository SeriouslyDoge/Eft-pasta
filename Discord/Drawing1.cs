...
public static class Drawing1
{
	...
	public static void DrawCircle(Vector2 center, float radius, Color color, float width, bool antiAlias, int segmentsPerQuarter)
	{
		float num = radius / 2f;
		Vector2 vector;
		vector..ctor(center.x, center.y - radius);
		Vector2 endTangent;
		endTangent..ctor(center.x - num, center.y - radius);
		Vector2 startTangent;
		startTangent..ctor(center.x + num, center.y - radius);
		Vector2 vector2;
		vector2..ctor(center.x + radius, center.y);
		Vector2 endTangent2;
		endTangent2..ctor(center.x + radius, center.y - num);
		Vector2 startTangent2;
		startTangent2..ctor(center.x + radius, center.y + num);
		Vector2 vector3;
		vector3..ctor(center.x, center.y + radius);
		Vector2 startTangent3;
		startTangent3..ctor(center.x - num, center.y + radius);
		Vector2 endTangent3;
		endTangent3..ctor(center.x + num, center.y + radius);
		Vector2 vector4;
		vector4..ctor(center.x - radius, center.y);
		Vector2 startTangent4;
		startTangent4..ctor(center.x - radius, center.y - num);
		Vector2 endTangent4;
		endTangent4..ctor(center.x - radius, center.y + num);
		Drawing1.DrawBezierLine(vector, startTangent, vector2, endTangent2, color, width, antiAlias, segmentsPerQuarter);
		Drawing1.DrawBezierLine(vector2, startTangent2, vector3, endTangent3, color, width, antiAlias, segmentsPerQuarter);
		Drawing1.DrawBezierLine(vector3, startTangent3, vector4, endTangent4, color, width, antiAlias, segmentsPerQuarter);
		Drawing1.DrawBezierLine(vector4, startTangent4, vector, endTangent, color, width, antiAlias, segmentsPerQuarter);
	}
	...
	public static void Initialize()
	{
		...
		if (flag)
		{
			Drawing1.lineTex = new Texture2D(1, 1, 5, false);
		...
		if (flag2)
		{
			Drawing1.aaLineTex = new Texture2D(1, 3, 5, false);
